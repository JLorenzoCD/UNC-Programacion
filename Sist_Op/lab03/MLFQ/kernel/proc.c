...
found:
  ...
  // The process is established with maximum priority and with the number of
  // times selected equal to zero.
  p->priority = NPRIO - 1;
  p->times_selected = 0;
...
scheduler(void)
{
  ...
  for(;;){
    ...
    // Se busca algun proceso que sea RUNNABLE
    ...
    // It should have changed its prio_p->state before coming back.
    c->proc = 0;
    release(&prio_p->lock);
  }
}
...
procdump(void)
{
  ...
  printf("\n");
  printf("PID\tSTATE\tNAME\tPRIORITY\tTIMES_SELECTED\n");
  for(p = proc; p < &proc[NPROC]; p++){
    if(p->state == UNUSED)
      continue;
    ...
    printf(
      "%d\t%s\t%s\t[%d]\t\t[#%d]",
      p->pid, state, p->name, p->priority, p->times_selected
      );
    printf("\n");
  }
}
