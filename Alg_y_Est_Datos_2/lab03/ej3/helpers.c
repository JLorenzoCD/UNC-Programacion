...
/**
 * @brief counts the amaount of players which have the same name as 'value'
 * present in Array 'a'
 *
 ...
 */
static unsigned int array_value_count(player_t a[], unsigned int length,
  player_t value) {
...
/**
 * @brief reads characters from file until feof or separator is reached. read
 * characters are stored in 'result'
 *
 ...
 */
static unsigned int process_string(FILE *file, char separator, char result[],
  unsigned int result_max_size) {
  unsigned int str_size = 0u;
  char c = (char)((int)separator + 1); // TODO: Preguntar sobre esto
...
void atp_dump(player_t atp[], unsigned int length) {
  for (unsigned int i = 0u; i < length; i++) {
    ...
    fprintf(stdout, "%u %u %u %u\n", atp[i].rank, atp[i].age, atp[i].points,
      atp[i].tournaments);
  }
}

/**
 * @brief process an opened file stream pointer and stores parsed values into
 * atp array
 *
