...
$$
⌊q_0B |^{x_1} B \cdots B |^{x_n} B \alpha_1 B \cdots B \alpha_m ⌋ \vdash^* ⌊p B |^{f(\overrightarrow{x}, \overrightarrow{\alpha})}⌋
$$
y $⌊p B |^{f(\overrightarrow{x}, \overrightarrow{\alpha})}⌋ \nvdash d$, para cada $d \in Des$

2. Si $(\overrightarrow{x}, \overrightarrow{\alpha}) \in ((\omega^n \times \Sigma^{*m}) − D_f)$, equivalentemente $(\overrightarrow{x}, \overrightarrow{\alpha}) \notin D_f$, entonces $M$ no se detiene partiendo de
$$
